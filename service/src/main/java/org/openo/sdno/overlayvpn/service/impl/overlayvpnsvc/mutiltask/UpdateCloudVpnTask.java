...
public class UpdateCloudVpnTask extends AbstCloudVpnTask {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(UpdateCloudVpnTask.class);
     ...
     * @param overlayVpn OverlayVpn need to Update
     */
    public UpdateCloudVpnTask(HttpServletRequest req, HttpServletResponse resp, String vpnTechnology,
    ...
    public void doTask() {
        ...
        LOGGER.info("UpdateCloudVpnTask vpn:" + this.overlayVpn.getUuid() + ", tec: " + this.vpnTechnology);

        createNewOverlayVpnInstance();

        try {
            if(CommConst.VPN_TEC_VXLAN.equals(this.vpnTechnology)) {
                this.execuRet = VxLANService.getInstance().update(httpRequest, this.overlayVpn);
            } else if(CommConst.VPN_TEC_IPSEC.equals(this.vpnTechnology)) {
                this.execuRet = IpSecService.getInstance().update(httpRequest, this.overlayVpn);
            } else {
                LOGGER.error("vpn technology invalied. tec: " + this.vpnTechnology);
                this.execuRet = new ResultRsp<>(ErrorCode.OVERLAYVPN_FAILED);
            }
        } catch(ServiceException updateException) {
            LOGGER.error("UpdateCloudVpnTask exception", updateException);
            this.execuRet = new ResultRsp<>(ErrorCode.OVERLAYVPN_FAILED);
        }
    ...
    public String getTaskName() {
        return "Overlayvpn service update CloudVpn Task. uuid: " + this.overlayVpn.getUuid() + ", tec: "
                + this.vpnTechnology;
    }
